...
def send_request(evalscript, input_data, coords, config, save_data):
    ...
    request = SentinelHubRequest(data_folder='../data/images', evalscript=evalscript, input_data=input_data, bbox=bbox, config=config, size=[512, 512], responses=[SentinelHubRequest.output_response('default', MimeType.PNG)])
